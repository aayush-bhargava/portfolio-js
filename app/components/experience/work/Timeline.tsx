...
const TimelinePoint = ({ point, diff }: { point: WorkTimelinePoint, diff: number }) => {
  ...
  const textProps: Partial<TextProps> = useMemo(() => ({
    ...
    color: "white",
    anchorX: textAlign,
    fillOpacity: 1, // Solid white for maximum visibility against bold backdrop
  }), [textAlign]);
  ...
  return (
    <group position={point.point} scale={isMobile ? 0.4 : 0.7}>
      ...
      <group>
        <group position={getPoint}>
          <Text {...textProps} fontSize={0.25} position={[0, 0.9, 0]}>
            {point.year}
          </Text>
          <group position={[0, 0, 0]}>
            <Text {...titleProps} position={[0, 0.1, 0]}>
              {point.title}
            </Text>
            <Text {...textProps} fontSize={0.22} position={[0, -0.7, 0]}>
              {point.subtitle}
            </Text>
          </group>
        </group>
